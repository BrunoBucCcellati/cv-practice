...
class DetectionEvaluator:
    ...
    @staticmethod
    def _iou(box_a: Tuple[int, int, int, int], box_b: Tuple[int, int, int,
                                                            int]) -> float:
    ...
    def evaluate_frame(self,
        ...
        candidate_detections = sorted(
            [d for d in detections if d.class_name == self.target_class],
        ...
        frame_tpr = frame_tp / (frame_tp + frame_fn) if (frame_tp +
                                                         frame_fn) > 0 else 0.0
        frame_fdr = frame_fp / (frame_tp + frame_fp) if (frame_tp +
                                                         frame_fp) > 0 else 0.0
    ...
    def get_metrics(self) -> Tuple[float, float]:
        ...
        tpr = self.true_positives / (self.true_positives +
                                     self.false_negatives)
        fdr = self.false_positives / (self.true_positives +
