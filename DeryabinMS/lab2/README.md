...
- формирование 4D-тензора через cv.dnn.blobFromImage.
...
- каждая детекция содержит:
  ...
  - оценку наличия объекта p_{object},
...
- изменение размера до 600x600 пикселей (используется версия модели ssd_mobilenet_v2_coco_2018_03_29, адаптированная для данного разрешения);
- конвертация изображения в 4-мерный тензор. *Примечание: в отличие от некоторых других реализаций, данная модель не требует нормализации пикселей в диапазон [-1, 1].*.
...
- координаты масштабируются:
  $$x_{px} = x \cdot W, \quad y_{px} = y \cdot H$$
...
- каждой детекции ставится в соответствие наилучший GT-бокс, если IoU ≥ 0.5;
- обновляются глобальные счётчики TP, FP, FN;
- вычисляются TPR/FDR для кадра и TPR/FDR по всем кадрам.
...
## Запуск приложения
Пример запуска из папки с проектом:
